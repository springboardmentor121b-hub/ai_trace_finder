...
def batch_corr_gpu(residuals, fingerprints, fp_keys):
    """
    Compute normalised cross-correlation for a batch of residuals against all fingerprints.
    Returns: (N, K)
    """
    ...
    # Prepare Fingerprints Matrix (K, D)
    fp_matrix = []
    for k in fp_keys:
        fp = fingerprints[k].astype(np.float32).ravel()
        fp -= fp.mean()
    ...
    res_matrix = np.array(res_matrix, dtype=np.float32)

    # Matrix multiplication (GPU / CPU)
    with tf.device('/GPU:0' if USE_GPU else '/CPU:0'):
        A = tf.convert_to_tensor(res_matrix)
        B = tf.convert_to_tensor(fp_matrix)
        C = tf.matmul(A, B)
