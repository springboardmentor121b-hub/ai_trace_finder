...
MODEL_PATH = os.path.join(ART_DIR, "hybrid_model.keras")
...
for dataset_name in ["official", "Wikipedia"]:
    ...
    print(f"Scanners in {dataset_name}:", list(residuals_dict[dataset_name].keys()))
    for scanner, dpi_dict in residuals_dict[dataset_name].items():
        print(f"Scanner {scanner}, type of dpi_dict: {type(dpi_dict)}")
        if isinstance(dpi_dict, dict):
            print(f"DPI keys: {list(dpi_dict.keys())}")
            for dpi, res_list in dpi_dict.items():
                print(f"DPI {dpi}, res_list length: {len(res_list)}")
                for res in res_list:
                    X_img_all.append(np.expand_dims(res, -1))
                    
                    if precomputed:
                        f_p = feats_prnu[idx_counter]
                        X_feat_all.append(f_p)
                    else:
        ...
        elif isinstance(dpi_dict, list):
            print(f"Direct list length: {len(dpi_dict)}")
            for res in dpi_dict:
                X_img_all.append(np.expand_dims(res, -1))
